...
export default function ETFMAssessment() {
  ...
  const handleFreeSubmit = async (e) => {
    ...
    try {
      const res  = await fetch("/api/claude", {
        ...
        body: JSON.stringify({
          type: "subscribe", firstName, email,
          answers: freeAnswers.map((value, i) => ({
            question: FREE_QUESTIONS[i].bot, answer: value,
          })),
        }),
      });
      await res.json();
      setScreen("snapshot_confirmed");   // ← goes to confirmation page
    } catch (err) {
  ...
  const handleBlueprintStart = async () => {
    ...
    try {
      const res  = await fetch("/api/blueprint-session", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "create", email, firstName,
          freeAnswers: freeAnswers.reduce((acc, val, i) => {
            acc[FREE_QUESTIONS[i].id] = val; return acc;
          }, {}),
        }),
      });
      const data = await res.json();
      ...
      await fetch("/api/blueprint-session", {
        ...
        body: JSON.stringify({
          ...
          blueprintAnswers: answers.map((value, i) => ({
            question: BLUEPRINT_QUESTIONS[i].bot, answer: value,
          })),
        }),
      });
    } catch (err) {
  ...
  const GoldLabel = ({ text }) => (
    <p style={{ fontSize: "11px", textTransform: "uppercase", letterSpacing: "3px", color: C.gold, marginBottom: "16px" }}>{text}</p>
  );
...
if (screen === "snapshot_confirmed") return (
  ...
  if (screen === "intro") return (
    <PageWrap maxWidth="600px">
      ...
      <h1 style={{ fontSize: "42px", fontFamily: "Georgia, serif", marginBottom: "20px", color: C.text, lineHeight: "1.2" }}>
        Your financial life has a pattern. It's time to see it clearly.
      </h1>
      <p style={{ fontSize: "17px", color: C.muted, marginBottom: "40px", lineHeight: "1.7" }}>
      ...
      <button onClick={() => setScreen("chat")} style={{ backgroundColor: C.gold, color: C.dark, border: "none", padding: "16px 44px", fontSize: "16px", fontWeight: "bold", borderRadius: "8px", cursor: "pointer" }}>
        Begin Your Snapshot →
      </button>
      ...
      <div style={{ minHeight: "100vh", backgroundColor: C.bg, padding: "40px 20px" }}>
        <div style={{ maxWidth: "680px", margin: "0 auto" }}>
          ...
          {q.options.map((opt, idx) => (
            <button key={idx} onClick={() => selectFreeAnswer(opt.value)} style={optionBtn}
  ...
  // ── TRANSITION (score reveal + email capture) ─────────────────────────────
...
Your Financial Snapshot, Archetype, and Awareness Score have already been delivered to your inbox. Your next step is choosing to get additional clarity and direction.
  Your Snapshot has been delivered to your inbox. Choose your next step below.
    ...
    <PageWrap>
      ...
      <p style={{ fontSize: "15px", color: C.muted, lineHeight: "1.7", marginBottom: "40px" }}>
        When you're ready to go deeper, choose your next step below.
      </p>
      ...
      <div style={{ display: "flex", gap: "16px", flexDirection: "column" }}>
        ...
        {/* $47 Blueprint */}
        <div style={{ backgroundColor: C.dark, borderRadius: "10px", padding: "28px", textAlign: "left" }}>
          <p style={{ color: C.gold, fontSize: "11px", textTransform: "uppercase", letterSpacing: "2px", marginBottom: "8px" }}>Strategic Blueprint</p>
          <h3 style={{ color: C.white, fontFamily: "Georgia, serif", fontSize: "20px", marginBottom: "12px", lineHeight: "1.3" }}>Go deeper with the 18-question diagnostic</h3>
          <p style={{ color: "#a0a0b8", fontSize: "14px", marginBottom: "16px", lineHeight: "1.7" }}>
            Your Snapshot shows where you are. The Blueprint identifies exactly why — and delivers your personalized escape roadmap and 30-Day Reset Protocol.
          </p>
          <p style={{ color: C.gold, fontSize: "13px", marginBottom: "16px", fontStyle: "italic" }}>
            ★ Your $47 will be credited toward the $499 Strategic Reset session if you upgrade.
          </p>
          <button onClick={() => setScreen("blueprint_bridge")}
            style={{ width: "100%", padding: "14px", backgroundColor: C.gold, color: C.dark, border: "none", borderRadius: "6px", fontWeight: "bold", fontSize: "15px", cursor: "pointer" }}>
            Learn More — Strategic Blueprint $47 →
          </button>
        </div>

        {/* $499 Session */}
        <div style={{ backgroundColor: C.white, border: `2px solid ${C.gold}`, borderRadius: "10px", padding: "28px", textAlign: "left" }}>
          <p style={{ color: C.gold, fontSize: "11px", textTransform: "uppercase", letterSpacing: "2px", marginBottom: "8px" }}>Premium</p>
          <h3 style={{ color: C.dark, fontFamily: "Georgia, serif", fontSize: "20px", marginBottom: "12px", lineHeight: "1.3" }}>Work directly with Robert</h3>
          <p style={{ color: C.muted, fontSize: "14px", marginBottom: "16px", lineHeight: "1.7" }}>
            A private 60-minute Strategic Reset Session with Robert Brickey. Built around your actual numbers, goals, and financial structure.
          </p>
          <button onClick={() => setScreen("session_bridge")}
            style={{ width: "100%", padding: "14px", backgroundColor: C.white, color: C.gold, border: `2px solid ${C.gold}`, borderRadius: "6px", fontWeight: "bold", fontSize: "15px", cursor: "pointer" }}>
            Learn More — Strategic Reset Session $499 →
          </button>
        </div>
      </div>
  ...
  // ── BLUEPRINT BRIDGE ──────────────────────────────────────────────────────
  // Explains the $47 Blueprint before going to Stripe
  if (screen === "blueprint_bridge") return (
    <PageWrap>
      <GoldLabel text="Strategic Blueprint" />
      <h2 style={{ fontSize: "34px", fontFamily: "Georgia, serif", color: C.text, marginBottom: "16px", lineHeight: "1.3" }}>
        Here's what happens next.
      </h2>
      <p style={{ fontSize: "16px", color: C.muted, lineHeight: "1.7", marginBottom: "32px" }}>
        After completing your payment you'll answer 18 deeper diagnostic questions. Then Robert's team generates your personalized Strategic Blueprint report and delivers it to your inbox within a few minutes.
      </p>

      <div style={{ backgroundColor: C.dark, borderRadius: "10px", padding: "28px", textAlign: "left", marginBottom: "20px" }}>
        <p style={{ color: C.gold, fontSize: "11px", textTransform: "uppercase", letterSpacing: "2px", marginBottom: "16px" }}>What's included</p>
        {[
          ...
          <div key={item} style={{ padding: "8px 0", color: "#c8c8d8", fontSize: "15px", lineHeight: "1.6", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            ✓ &nbsp;{item}
          </div>
        ))}
        <div style={{ marginTop: "20px", padding: "14px", backgroundColor: "rgba(201,151,58,0.12)", borderRadius: "6px", border: "1px solid rgba(201,151,58,0.3)" }}>
          <p style={{ color: C.gold, fontSize: "14px", margin: 0, lineHeight: "1.6" }}>
            ★ <strong>Upgrade Credit:</strong> Your $47 will be fully credited toward the $499 Strategic Reset Session if you choose to work directly with Robert after receiving your Blueprint.
          </p>
        </div>
      </div>

      <button onClick={handleBlueprintStart} disabled={loading}
        style={{ width: "100%", padding: "16px", backgroundColor: loading ? C.muted : C.gold, color: C.dark, border: "none", borderRadius: "6px", fontWeight: "bold", fontSize: "16px", cursor: loading ? "not-allowed" : "pointer", marginBottom: "12px" }}>
        {loading ? "Setting up your Blueprint..." : "Get the Strategic Blueprint — $47"}
      </button>
      <button onClick={() => setScreen("snapshot_confirmed")}
        style={{ width: "100%", padding: "12px", backgroundColor: "transparent", color: C.muted, border: `1px solid ${C.border}`, borderRadius: "6px", fontSize: "14px", cursor: "pointer" }}>
        ← Go back
      </button>
  ...
  // ── SESSION BRIDGE ────────────────────────────────────────────────────────
  // Explains the $499 session before going to Calendly
  if (screen === "session_bridge") return (
    <PageWrap>
      ...
      <p style={{ fontSize: "16px", color: C.muted, lineHeight: "1.7", marginBottom: "32px" }}>
        You'll book a private 60-minute session directly with Robert Brickey. Before your session, you'll complete a pre-session financial intake so Robert can review your situation and come prepared with a tailored plan.
      </p>
      ...
      <div style={{ backgroundColor: C.white, border: `1px solid ${C.border}`, borderRadius: "10px", padding: "28px", textAlign: "left", marginBottom: "20px" }}>
        <p style={{ color: C.gold, fontSize: "11px", textTransform: "uppercase", letterSpacing: "2px", marginBottom: "16px" }}>What's included — $499</p>
        {[
          ...
          <div key={item} style={{ padding: "8px 0", color: C.body, fontSize: "14px", lineHeight: "1.6", borderBottom: `1px solid ${C.border}` }}>
            ✓ &nbsp;{item}
          </div>
        ))}
        <div style={{ marginTop: "20px", padding: "14px", backgroundColor: C.goldSoft, borderRadius: "6px", border: `1px solid rgba(201,151,58,0.3)` }}>
          <p style={{ color: C.dark, fontSize: "14px", margin: 0, lineHeight: "1.6" }}>
            ★ <strong>Already purchased the Blueprint?</strong> Your $47 will be credited toward this session. Mention it when booking.
          </p>
        </div>
      </div>

      <a href={CALENDLY} target="_blank" rel="noopener noreferrer"
        style={{ display: "block", width: "100%", padding: "16px", backgroundColor: C.gold, color: C.dark, border: "none", borderRadius: "6px", fontWeight: "bold", fontSize: "16px", cursor: "pointer", textDecoration: "none", marginBottom: "12px", boxSizing: "border-box" }}>
    ...
    return (
      <div style={{ minHeight: "100vh", backgroundColor: C.bg, padding: "40px 20px" }}>
        <div style={{ maxWidth: "680px", margin: "0 auto" }}>
          ...
          {q.options.map((opt, idx) => (
            <button key={idx} onClick={() => selectBlueprintAnswer(opt.value)} style={optionBtn}
              onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = C.goldSoft; e.currentTarget.style.borderColor = C.gold; }}
              onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = C.white; e.currentTarget.style.borderColor = C.border; }}>
              {opt.label}
            </button>
      ...
      <div style={{ maxWidth: "620px", width: "100%" }}>
        ...
        <div style={{ backgroundColor: C.white, borderRadius: "12px", border: `1px solid ${C.border}`, overflow: "hidden" }}>
          ...
          <div style={{ padding: "40px" }}>
            <p style={{ margin: "0 0 16px", color: C.text, fontSize: "16px", lineHeight: "1.8" }}>You took an important step today.</p>
            ...
            <div style={{ backgroundColor: C.bg, borderRadius: "8px", padding: "24px 28px", marginBottom: "32px", borderLeft: `3px solid ${C.gold}` }}>
              ...
              <p style={{ margin: 0, color: C.muted, fontSize: "14px", lineHeight: "1.8" }}>
                Your personalized report, escape roadmap, 30-Day Reset Protocol, and Strategic Score breakdown will be delivered shortly. This typically takes 3–5 minutes. Check your spam folder if you don't see it.
              </p>
            </div>
            <hr style={{ border: "none", borderTop: `1px solid ${C.border}`, margin: "0 0 32px" }} />
            ...
            <div style={{ textAlign: "center", padding: "24px 0" }}>
              <p style={{ margin: 0, color: C.text, fontSize: "16px", lineHeight: "2.2", fontFamily: "Georgia, serif", fontStyle: "italic" }}>
                Clarity creates control.<br />Control creates momentum.<br />Momentum creates freedom.
              </p>
            </div>
          </div>
          <div style={{ backgroundColor: C.bg, padding: "20px 40px", textAlign: "center", borderTop: `1px solid ${C.border}` }}>
            <p style={{ margin: 0, color: C.muted, fontSize: "13px" }}>
              Questions? <a href="mailto:exit@etfm.systems" style={{ color: C.gold, textDecoration: "none" }}>exit@etfm.systems</a>
            </p>
          </div>
        </div>
      </div>
